...
DroneBatteryDecorator::DroneBatteryDecorator(Drone* drone,
    ...
    : DroneDecorator(drone), maxCharge(maxCharge_),
    currentCharge(currentCharge_), lowCharge(lowCharge_),
...
void DroneBatteryDecorator::decreaseCharge(double dt) {
    ...
    if (timeElapsed >= movingDecreaseTime) {
        ...
        if (createMessage) {
            std::string message = getName() + " now at " +
                std::to_string(currentCharge) + "% charge";
...
void DroneBatteryDecorator::charge(double dt) {
    if (malfunctionedStationTime > 0) {
        ...
        if (malfunctionedStationTime <= 0) {
            std::string message = "Station charging " +
                getName() + " has been fixed.";
            sub->notifyObservers(message);
...
void DroneBatteryDecorator::lookAheadForRechargeStation() {
    ...
    if (sub->getToPackageStrategy()) {
        ...
        distanceToBattery =
            strategy->currentPathDistance(currentPosition);
    ...
    if (batteryNeeded + lowCharge >= maxCharge) {
        ...
        if (currentCharge - distanceToBattery <= lowCharge) {
            ...
            " does not have enough charge to get to package, " +
...
void DroneBatteryDecorator::update(double dt) {
    if (currentCharge <= 0 && !isAtRechargeStation()) {
        for (auto i = getModel()->chargingDrones.begin();
             i != getModel()->chargingDrones.end(); ++i) {
            if (*i == this) {
                return;
            }
        }
    ...
    bool isToPackageValid =
        sub->getToPackageStrategy() != nullptr;
    if (goingToPackage && !isToPackageValid) {
        // Just picked up the package
        goingToPackage = false;
    }

    bool isFinalDestinationValid =
        sub->getToFinalDestinationStrategy() != nullptr;
    ...
    bool droneIsIdle = !isToPackageValid &&
                       !isFinalDestinationValid &&
                       !isAtRechargeStation() &&
                       toRechargeStation == nullptr;
    ...
    if (droneIsIdle) {
        ...
        // Drone is idle, so go to recharge station
            std::string message = getName() +
                " is idle, so heading to recharge station";
            sub->notifyObservers(message);
            headToRechargeStation(findNearestRechargeStation());
        } else {
            idleFrames += 1;
...
void DroneBatteryDecorator::addToDeadDronesList() {
    if (std::find(getModel()->deadDrones.begin(),
                  getModel()->deadDrones.end(), this) ==
        getModel()->deadDrones.end())
    getModel()->deadDrones.push_back(this);
}

void DroneBatteryDecorator::addToFunctionalDroneList() {
    if (std::find(getModel()->functionalDrones.begin(),
                  getModel()->functionalDrones.end(), this) ==
...
void DroneBatteryDecorator::removeFromFunctionalDroneList() {
    getModel()->functionalDrones.erase(
        std::remove(getModel()->functionalDrones.begin(),
                    getModel()->functionalDrones.end(), this),
        getModel()->functionalDrones.end());
}
...
void DroneBatteryDecorator::arriveAtRechargeStation() {
    ...
    if (randomNumber % 10 == 0) {
        ...
        std::string secondHalfMessage = "attempting to recharge" +
            getName() +
            ". Please wait 10 seconds for it to be fixed.";
