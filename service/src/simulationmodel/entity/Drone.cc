    ...
    if (toFinalDestination->isCompleted()) {
      ...
      if (portionNum > 2) {
        for (int i = 0; i < portionNum - 2; i++) {
          ...
          std::string message = getName() + " dropped off: "
          + package->getName() + " (Item " + portion + ")";
          notifyObservers(message);
        }
      }
...
IStrategy* Drone::getToPackageStrategy() { return toPackage; }

IStrategy* Drone::getToFinalDestinationStrategy() { return toFinalDestination; }

bool Drone::isAvailable() { return available; }

bool Drone::isPickedUp() { return pickedUp; }
