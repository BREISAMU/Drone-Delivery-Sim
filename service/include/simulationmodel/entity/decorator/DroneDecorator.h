...
class DroneDecorator : public IEntityDecorator<Drone> {
  ...
  */
   ...
   */
  virtual void getNextDelivery() const { return sub->getNextDelivery(); }

  /**
   * @brief Gets the pickedUp status of the Drone object
   */
  virtual bool getPickedUp() const { return sub->getPickedUp(); }

  /**
   * @brief Gets pointer to the Drone's package
   */
  virtual Package* getPackage() const { return sub->getPackage(); }
   ...
   */
  virtual void setToPackage(IStrategy* s) const { return sub->setToPackage(s); }
